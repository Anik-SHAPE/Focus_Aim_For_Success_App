...
const Signin = () => {
    ...
    const onSubmit = event => {
      ...
      const performRedirect = () => {
        if (didRedirect) {
          if (user && user.role === 2) {
            return <Redirect to={`/user/dashboard/courses`} />;
          }
        }
        if (isAuthenticated()) {
      ...
      const SigninForm = () => {
          return(
              ...
              <div className="container">
                &nbsp;&nbsp;
                <center><img src={Logo}  width="140" height="140"/></center>
                <Card  style={{backgroundColor: '#ffffff', borderRadius: 10}} className="m-lg-5 shadow" >
                    ...
                    <a href="/"><h6 style={{color: "#f50057"}}>Goto Home Page?</h6></a>
                        <h6 className="text-muted text-left">Don't have any accout? <a href="/signup">Signup</a></h6>                   
                    </div>                          
                    </div>        
                </Card>
                </div>
