...
import {getCourse, createPushSubjectToCourse, getAllSubjectByCourse, deleteSubject, getSubject, getAllQuestionBySubject, deleteQuestion, getAllChapterBySubject, deleteChapter, getChapter} from "./helper/adminapicall";
...
const ManageChapter = (props) => {
     ...
     <center><img class="pb-5" src={Logo} width="140" height="190"/></center>
      ...
      <list>
        <ul class="navbar-nav">
          ...
          <a href={`/manage/${props.match.params.courseId}/${props.match.params.subjectId}/chapter/create`} style={{textDecoration: 'none'}}>
          <li class="nav-item active">
            <ListItem Button >
              <ListItemIcon>
                  <ImportContactsRoundedIcon style={{color: pink[500]}}/>
  ...
  const GetChapter = () => {
    ...
    const deleteAchapter = chapterId => {
        deleteChapter(user._id, token, props.match.params.subjectId, chapterId).then(data => {
            if(data.error){
                console.log(data.error);
            }else{
                chapterload();
                          ...
                          <Grid item xs={12} sm={6} md={8} >
                            <div className=" m-2 pl-4 pr-4 d-block w-100 h-100">
                              <h6 className="my-auto font-weight-bold" style={{color: "#ffa000"}}>
                                  {chapter.cname}</h6>
                              <div className="mt-3">
                              <Link to={`/manage/${props.match.params.subjectId}/${chapter._id}/contents/intro`}>
                                  <button className="btn btn-outline-info shadow-sm"  style={{ borderRadius: 5
                                    }}>
                                      <center className="text-black">Manage Contents</center>
                                  </button>
                              </Link>
                              <Link to={`/manage/${props.match.params.courseId}/${props.match.params.subjectId}/${chapter._id}/chapter/update`}>
                              ...
                              <button onClick={() => {
                                confirmAlert({
                                  title: 'Delete:'+" "+chapter.cname,
                                  message: 'Are you sure you want to delete.',
                                  buttons: [
                                    {
                                      label: 'Yes',
                                      onClick: () => deleteAchapter(chapter._id)
                                    },
                                    {
                                      label: 'No',
                                      onClick: () => {}
