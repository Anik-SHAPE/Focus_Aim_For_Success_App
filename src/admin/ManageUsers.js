...
import { createUser, getAllNormalUser, delUser} from './helper/adminapicall';
...
function ResponsiveDrawer(props) {
        ...
        <li class="nav-item">
          <a href="/manage/users" style={{textDecoration: 'none'}}>
            <ListItem Button>
              ...
              <ListItemText style={{color: purple[800]}}  primary="Users"/>
            </ListItem>
          </a>
        </li>
  ...
  const preload = (userId, token) => {
    getAllNormalUser(userId, token).then(data => {
        if(data.error){
  ...
  const {firstname, lastname, email, phone, password, photo, role, error, publish, loading, success, formData} = values;
  ...
  const onSubmit = event => {
    ...
    // request to backend
    createUser(user._id, token, formData)
    .then(data => {
      if(data.error){
        ...
        <div className="row pt-3 pb-3" >
          <div className="col-md-6 offset-sm-3 text-left">
            <div className="alert alert-success shadow"  style={{ borderRadius: 30}}>
              <center> <h4 className="text-success my-auto">User Account Created Succesfully</h4> </center>
            </div>
          </div>
        </div>  
      )
      ...
      <AppBar style={{backgroundColor: '#0C090A', borderBottomLeftRadius: 10, borderBottomRightRadius: 10}} position="fixed" className={classes.appBar}>
        <Toolbar style={toolbarStyle}>
          ...
          <Typography variant="h6" noWrap >
           <h5 class="my-auto font-weight-bolder">User Management</h5>
          </Typography>
        </Toolbar>
      </AppBar>
        ...
        &nbsp; &nbsp;
            <Grid container spacing="4">
              ...
              <CardContent>
                  ...
                  <h5  style={{color: "#00b8d4"}} className=" font-weight-bold pb-2 pt-1">Create User</h5>
                  </Typography>
