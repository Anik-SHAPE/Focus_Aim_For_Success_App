...
import { getQuestion, updateQuestion, getChapter} from "../admin/helper/adminapicall";
...
const Content = (props) => {
        ...
        <ul class="navbar-nav">
          <li class="nav-item active">
            <ListItem Button >
              ...
              <ListItemText style={{color: purple[500]}} primary="Update Content"/>
            </ListItem>
          </li>
        <li class="nav-item">
  ...
  const ManageContent = () => {
    ...
    const contentload = (userId, token, questionId) => {
        getQuestion(userId, token, questionId).then(data=> {
            if(data.error){
    ...
    useEffect(() => {
        ...
        contentload(user._id, token, props.match.params.questionId);
    }, []);
    ...
    const onSubmit = event => {
        ...
        updateQuestion(user._id, token, props.match.params.questionId, formData)
        .then(data => {
            if(data.error){
                setcontents({...contents, error: data.error});
            }else{
            ...
            <div className="row pt-3 pb-3" >
              <div className="col-md-6 offset-sm-3 text-left">
                <div className="alert alert-success shadow"  style={{ borderRadius: 30}}>
                  <center> <h4 className="text-success my-auto">Content Updated Succesfully in {cname}</h4>
                  <h6 className=" pt-1 my-auto">Go to <a href={`/manage/writter/${props.match.params.subjectId}/${props.match.params.chapterId}/contents`} style={{textDecoration: "none"}}>Contents </a>to get the update</h6>
                    </center>
                </div>
              </div>
            </div>  
        ...
        <Grid container spacing="4">
          ...
          <Grid item xs={12} sm={12} md={12} >
              ...
              <CardContent>
                  ...
                  <h5 className="font-weight-bold pb-2 pt-1" style={{color: "#019031"}}>Update Content</h5>
                  </Typography>
